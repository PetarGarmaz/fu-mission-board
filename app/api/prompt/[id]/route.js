...
export const GET = async (req, {params}) => {
	try {
		...
		const briefing = await Briefing.findById(params.id).populate("host");
...
export const PATCH = async (req, {params}) => {
	const {title, host_date, briefing, briefing_image, status } = await req.json();
	...
	try {
		...
		existingBriefing.host_date = host_date;
		existingBriefing.briefing = briefing;
		existingBriefing.briefing_image = briefing_image;
